...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="questionnaire/list.do" method="post" name="Form" id="Form">
			<table>
				<tr>
									...
									<div class='hidden-phone visible-desktop btn-group'>
										...
										<ul class="dropdown-menu dropdown-icon-only dropdown-light pull-right dropdown-caret dropdown-close">
											<c:if test="${QX.edit == 1 }">
												...
												<c:if test="${var.STATUS eq '2' }">
													<li><a style="cursor:pointer;" title="启用" onclick="qiyong('${var.ID}');" class="tooltip-success" data-rel="tooltip" title="" data-placement="left"><span class="green"><i class="icon-check"></i></span></a></li>
												</c:if>
											</c:if>
											<c:if test="${QX.del == 1 }">
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>questionnaire/goAdd.do';
			 diag.Width = 900;
			 diag.Height = 710;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>questionnaire/delete.do?ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>questionnaire/goEdit.do?ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function qiyong(Id){
			top.jzts();
			var url = "<%=basePath%>questionnaire/qiyong.do?ID="+Id+"&tm="+new Date().getTime();
			$.get(url,function(data){
				nextPage(${page.currentPage});
			});
		}
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>questionnaire/excel.do';
		}
		</script>
	</body>
</html>
