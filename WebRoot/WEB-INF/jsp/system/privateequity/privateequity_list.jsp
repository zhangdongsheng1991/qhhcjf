...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="privateequity/list.do" method="post" name="Form" id="Form">
			<!-- 检索  -->
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						...
						<th class="center">项目全称</th>
						<th class="center">管理机构</th>
						<th class="center">存续期限</th>
						<th class="center">目标规模</th>
						<th class="center">投资门槛</th>
						<th class="center">投资方向</th>
						<th class="center">有无悔购</th>
						<th class="center">组织形式</th>
						<th class="center">募集状态</th>
						<th class="center">管 理 费</th>
						<th class="center">专家点评</th>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td class='center' style="width: 30px;">${vs.index+1}</td>
										<td class="center">${var.PROJECTFULLNAME}</td>
										<td class="center">${var.REGULATORYAGENCY}</td>
										<td class="center">${var.DURATION}</td>
										<td class="center">${var.TARGETSCALE}</td>
										<td class="center">${var.INVESTMENTTHRESHOLD}</td>
										<td class="center">${var.INVESTMENTORIENTATION}</td>
										<td class="center">${var.WHETHERORNOTTOBUY}</td>
										<td class="center">${var.ORGANIZATIONFORM}</td>
										<td class="center">${var.RECRUITMENTSTATUS}</td>
										<td class="center">${var.MANAGEMENTEXPENSE}</td>
										<td class="center">${var.EXPERTCOMMENT}</td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>privateequity/goAdd.do';
			 diag.Width = 900;
			 diag.Height = 710;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>privateequity/delete.do?ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>privateequity/goEdit.do?ID='+Id;
			 diag.Width = 900;
			 diag.Height = 710;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>privateequity/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>privateequity/excel.do';
		}
		</script>
		<script type="text/javascript">
			function changes(e){
