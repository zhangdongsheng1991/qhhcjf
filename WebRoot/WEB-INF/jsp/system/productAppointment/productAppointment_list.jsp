...
<div id="page-content" style="width: 98%;" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="productAppointment/list.do" method="post" name="Form" id="Form">
			<table>
				<tr>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							...
							<%-- 	<td class='center' style="width: 30px;">
								...
								<td class='center' style="width: 30px;">${vs.index+1}</td>
										...
										<td class="center">${var.CHANNELURL}</td>
										<td class="center">${var._PORT}</td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>productAppointment/goAdd.do';
			 diag.Width = 900;
			 diag.Height = 710;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>productAppointment/delete.do?ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>productAppointment/goEdit.do?ID='+Id;
			 diag.Width = 900;
			 diag.Height = 710;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function shenhe(Id){
			top.jzts();
			var url = "<%=basePath%>productAppointment/shenhe.do?ID="+Id+"&tm="+new Date().getTime();
			$.get(url,function(data){
				nextPage(${page.currentPage});
			});
		}
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>productAppointment/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>productAppointment/excel.do';
		}
		</script>
	</body>
</html>
