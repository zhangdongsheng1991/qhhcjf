...
<link rel="Shortcut Icon" href="static/img/web/logo2.ico"> 
 <title>契约型理财产品_契约型理财产品咨询_契约型理财产品预约 - 环成金服</title>
<meta name="keywords" content="契约型理财产品,契约型理财产品咨询,专业契约型理财咨询,契约型产品简介,契约型理财收益,契约型理财增信措施,契约型理财产品风险">
<meta name="description" content="环成金服--一站式契约型理财服务平台,专业理财顾问全天候24小时提供契约型理财咨询;全面解读契约型产品收益风险,免费咨询,可以依据客户偏好、为客户量身定制个性化契约型理财产品。">
...
<body style=" background-color:#fcfcfc;">
  <script type="text/javascript">
	...
	<input type="hidden" id="WEBTYPE" value="4">
   <div id="container" style="border: none; background: none;">
    ...
    <div id="common_currentlocation" class=" clearb">
        <div id="ctl00_ContentPlaceHolder1_Location_common_currentlocation_list" class=" w1200 mauto h48 lh48 fs14 c666666">
			...
			<div class=" fl ml10">&gt;</div><div class=" fl ml10"><a href="javascript:{location.reload();}">契约产品</a></div>
		</div>
    </div>   
     <!-- 产品搜索 -->
            ...
            <div class=" w1200 mauto">
                ...
                <div class="h44 bbs0068ff fs18">
                    ...
                    <div id="ctl00_ContentPlaceHolder1_trust_ProductCenter_Search_link" class=" h24">
                    	<div class=" fwb w120 h24 fl brsdedede tac"><a href="web/trust" class=" w120 h24 ">信托产品</a></div>
						<div class=" fwb w120 h24 fl brsdedede tac"><a href="web/mislist" class=" w120 h24 ">资管产品</a></div>
						<div class=" fwb w170 h24 fl tac"><a  style="color: #f49703"  href="javascript:{location.reload();}" class=" w120 h24 c3a8eff">契约型产品</a></div>
					</div>
                </div>
                <!-- 搜索区域 -->
				...
				<c:when test="${not empty list}">
						<c:forEach items="${list}" var="var" varStatus="vs">
							<tr class="list_info bbdebebeb c999999">
								...
								<td class=" h32 mt8"><a href="web/goProductInfo?ID=${var.ID}&PRODUCTTYPE=1" target="_blank" class=" c666666"><div class=" fl w32 h32"><img src="${var.THEISSUERIMGURL }" width="32" height="32"></div><div class=" fl h32 lh32 ml5">
...
<%@include file="bottom.jsp"%>
    ...
    function jsCopy(i,user){
 		...
 		$(document).on('click', '.collection', function(e) {
 			...
 			if(imgname=="nocollection"){
     			...
     			$.ajax({
     				...
     				data:{'productId':productId,'collection':'no','phone':phone,'product_type':'qyx','TYPE':'3'},
     				dataType:'json',
		...
		function getList(currentPage){
			...
			$.ajax({
				type: "POST",
				url: '<%=basePath%>web/getCovenantList',
		    	data: data,
				dataType:'json',
				success: function(data){
